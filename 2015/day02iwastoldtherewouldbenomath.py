...
def doTheMath():   
    ...
    for box in wrappingPaperSizes:
        ...
        side3 = 2*(box[0]*box[2])
        slack = box[0]*box[1]
        totalWrappingPaper = totalWrappingPaper + side1 + side2 + side3 + slack
        smallestPerimeter = 2*box[0] + 2*box[1]
        cubicFeet = box[0]*box[1]*box[2]
        totalRibbon = totalRibbon + smallestPerimeter + cubicFeet
    print(f"The elves need to order {totalWrappingPaper} square feet of \
wrapping paper and {totalRibbon} feet of ribbon.")
